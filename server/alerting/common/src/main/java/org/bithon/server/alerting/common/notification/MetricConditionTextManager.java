...
public class MetricConditionTextManager {

    private final LoadingCache<Key, String> textCache;

    public MetricConditionTextManager(IDataSourceApi dataSourceApi) {
        this.textCache = Caffeine.newBuilder()
                                 .expireAfterWrite(Duration.ofHours(1))
                                 .build(key -> key.getCondition()
                                                  .accept(new MetricConditionTextBuilder(dataSourceApi.getSchemaByName(key.getDataSource()))));
    }

    public String getDisplayText(String dataSource, IMetricCondition condition) {
        return textCache.get(new Key(dataSource, condition));
    }
    ...
    private static class Key {
        ...
        private IMetricCondition condition;
    }
