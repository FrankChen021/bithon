...
public class AlertQueryApi {
    ...
    public ApiResponse<AlertRecordObject> getAlertRecordById(@Valid @RequestBody GetAlertRecordByIdRequest request) {
        return ApiResponse.success(alertRecordStorage.getAlertRecord(request.getId()));
    }

    @PostMapping("/alerting/api/alert/getRecords")
    public GetAlertRecordListResponse getRecords(@Valid @RequestBody GetAlertRecordListRequest request) {
        ...
        return new GetAlertRecordListResponse(results.getRows(), results.getData().stream().map(obj -> {
            ...
            bo.setServerCreateTime(obj.getTimestamp());
            return bo;
        }).collect(Collectors.toList()));
    }
    ...
    public GetChangeLogListResponse getChangeLogs(@Valid @RequestBody GetAlertChangeLogListRequest request) {
        ...
        return new GetChangeLogListResponse(results.getRows(), results.getData().stream().map(log -> {
            ChangeLogBo bo = new ChangeLogBo();
            BeanUtils.copyProperties(log, bo);
            bo.setTimestamp(log.getServerUpdateTime().getTime());
            return bo;
        }).collect(Collectors.toList()));
    }

    @PostMapping("/alerting/api/alert/getRunningLogs")
    public GetEvaluationLogsResponse getRunningLogs(@Valid @RequestBody GetEvaluationLogsRequest request) {
