...
public class AlertObjectLoader {
    ...
    public void loadChanges() {
        ...
        alertObjects.stream().filter(AlertStorageObject::getDeleted).forEach((alertObject) -> {
        ...
        //从Payload获取告警配置
        //
        alertObjects.stream().filter(alert -> !alert.getDeleted()).map(this::toAlert).filter(Objects::nonNull)
                    .forEach((alert) -> {
