...
import org.bithon.server.alerting.processor.service.AlertObjectLoader;
...
public class AlertEvaluatorScheduler {
    ...
    public void onSchedule() {
        ...
        TimeSpan now = TimeSpan.nowInMinute();
        for (Alert alert : alertObjectLoader.getLoadedAlerts().values()) {
            executor.execute(() -> {
                alertEvaluator.evaluate(now, alert);
            });
        }
    }
    ...
    private void evaluate(String alertId) {
        ...
        if (alert != null) {
            alertEvaluator.evaluate(TimeSpan.nowInMinute(), alert);
        }
    }
}
