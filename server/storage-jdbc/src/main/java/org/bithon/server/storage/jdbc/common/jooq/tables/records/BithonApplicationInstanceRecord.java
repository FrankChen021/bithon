/*
 * This file is generated by jOOQ.
 */
package org.bithon.server.storage.jdbc.common.jooq.tables.records;


import java.time.LocalDateTime;

import org.bithon.server.storage.jdbc.common.jooq.tables.BithonApplicationInstance;
import org.jooq.Field;
import org.jooq.Record4;
import org.jooq.Row4;
import org.jooq.impl.TableRecordImpl;


/**
 * This class is generated by jOOQ.
 */
@SuppressWarnings({ "all", "unchecked", "rawtypes" })
public class BithonApplicationInstanceRecord extends TableRecordImpl<BithonApplicationInstanceRecord> implements Record4<LocalDateTime, String, String, String> {

    private static final long serialVersionUID = 1L;

    /**
     * Setter for <code>bithon_application_instance.timestamp</code>. update
     * time
     */
    public void setTimestamp(LocalDateTime value) {
        set(0, value);
    }

    /**
     * Getter for <code>bithon_application_instance.timestamp</code>. update
     * time
     */
    public LocalDateTime getTimestamp() {
        return (LocalDateTime) get(0);
    }

    /**
     * Setter for <code>bithon_application_instance.appName</code>.
     */
    public void setAppname(String value) {
        set(1, value);
    }

    /**
     * Getter for <code>bithon_application_instance.appName</code>.
     */
    public String getAppname() {
        return (String) get(1);
    }

    /**
     * Setter for <code>bithon_application_instance.appType</code>.
     */
    public void setApptype(String value) {
        set(2, value);
    }

    /**
     * Getter for <code>bithon_application_instance.appType</code>.
     */
    public String getApptype() {
        return (String) get(2);
    }

    /**
     * Setter for <code>bithon_application_instance.instanceName</code>.
     */
    public void setInstancename(String value) {
        set(3, value);
    }

    /**
     * Getter for <code>bithon_application_instance.instanceName</code>.
     */
    public String getInstancename() {
        return (String) get(3);
    }

    // -------------------------------------------------------------------------
    // Record4 type implementation
    // -------------------------------------------------------------------------

    @Override
    public Row4<LocalDateTime, String, String, String> fieldsRow() {
        return (Row4) super.fieldsRow();
    }

    @Override
    public Row4<LocalDateTime, String, String, String> valuesRow() {
        return (Row4) super.valuesRow();
    }

    @Override
    public Field<LocalDateTime> field1() {
        return BithonApplicationInstance.BITHON_APPLICATION_INSTANCE.TIMESTAMP;
    }

    @Override
    public Field<String> field2() {
        return BithonApplicationInstance.BITHON_APPLICATION_INSTANCE.APPNAME;
    }

    @Override
    public Field<String> field3() {
        return BithonApplicationInstance.BITHON_APPLICATION_INSTANCE.APPTYPE;
    }

    @Override
    public Field<String> field4() {
        return BithonApplicationInstance.BITHON_APPLICATION_INSTANCE.INSTANCENAME;
    }

    @Override
    public LocalDateTime component1() {
        return getTimestamp();
    }

    @Override
    public String component2() {
        return getAppname();
    }

    @Override
    public String component3() {
        return getApptype();
    }

    @Override
    public String component4() {
        return getInstancename();
    }

    @Override
    public LocalDateTime value1() {
        return getTimestamp();
    }

    @Override
    public String value2() {
        return getAppname();
    }

    @Override
    public String value3() {
        return getApptype();
    }

    @Override
    public String value4() {
        return getInstancename();
    }

    @Override
    public BithonApplicationInstanceRecord value1(LocalDateTime value) {
        setTimestamp(value);
        return this;
    }

    @Override
    public BithonApplicationInstanceRecord value2(String value) {
        setAppname(value);
        return this;
    }

    @Override
    public BithonApplicationInstanceRecord value3(String value) {
        setApptype(value);
        return this;
    }

    @Override
    public BithonApplicationInstanceRecord value4(String value) {
        setInstancename(value);
        return this;
    }

    @Override
    public BithonApplicationInstanceRecord values(LocalDateTime value1, String value2, String value3, String value4) {
        value1(value1);
        value2(value2);
        value3(value3);
        value4(value4);
        return this;
    }

    // -------------------------------------------------------------------------
    // Constructors
    // -------------------------------------------------------------------------

    /**
     * Create a detached BithonApplicationInstanceRecord
     */
    public BithonApplicationInstanceRecord() {
        super(BithonApplicationInstance.BITHON_APPLICATION_INSTANCE);
    }

    /**
     * Create a detached, initialised BithonApplicationInstanceRecord
     */
    public BithonApplicationInstanceRecord(LocalDateTime timestamp, String appname, String apptype, String instancename) {
        super(BithonApplicationInstance.BITHON_APPLICATION_INSTANCE);

        setTimestamp(timestamp);
        setAppname(appname);
        setApptype(apptype);
        setInstancename(instancename);
    }
}
