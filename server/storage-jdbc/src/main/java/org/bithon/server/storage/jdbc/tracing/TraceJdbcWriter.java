...
public class TraceJdbcWriter implements ITraceWriter {
    ...
    public static String getInsertSpanTableStatement(DSLContext dslContext, Table<?> table) {
        return dslContext.render(dslContext.insertInto(table,
    ...
    public abstract static class InsertSpanRunnable implements IInsert {
        private final String insertStatement;
        private final List<TraceSpan> spans;
        private final String table;

        public InsertSpanRunnable(DSLContext dslContext,
                                  Table<?> table,
                                  List<TraceSpan> spans) {
            this.insertStatement = getInsertSpanTableStatement(dslContext, table);
    ...
    private void writeSpans(List<TraceSpan> traceSpans, Table<?> table) throws Throwable {
        if (!traceSpans.isEmpty()) {
            doInsert(createInsertSpanRunnable(table, traceSpans));
        }
    }

    protected void doInsert(IInsert runnable) throws Throwable {
        try {
    ...
    public static class InsertMappingRunnable implements ConnectionRunnable {
        private final String insertStatement;
        private final Collection<TraceIdMapping> mappings;

        public InsertMappingRunnable(DSLContext dslContext,
    ...
    public static class InsertIndexRunnable implements ConnectionRunnable {
        private final String insertStatement;
        private final Collection<Object[]> tagIndices;

        public InsertIndexRunnable(DSLContext dslContext, Collection<Object[]> tagIndices) {
    ...
    private void writeTagIndices(Collection<TagIndex> tagIndices) {
        ...
        dslContext.connection(createInsertIndexRunnable(dslContext, batchValues.values()));
    }
