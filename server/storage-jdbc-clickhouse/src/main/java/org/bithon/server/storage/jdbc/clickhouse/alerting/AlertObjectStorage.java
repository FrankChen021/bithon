...
public class AlertObjectStorage extends AlertObjectJdbcStorage {
    ...
    @JsonCreator
    public AlertObjectStorage(@JacksonInject(useInput = OptBoolean.FALSE) ClickHouseJooqContextHolder dslContextHolder,
    ...
    public void initialize() {
        new TableCreator(this.config, this.dslContext).useReplacingMergeTree(true)
                                                      .replacingMergeTreeVersion(Tables.BITHON_ALERT_OBJECT.UPDATED_AT.getName())
                                                      .partitionByExpressioin(null)
                                                      .createIfNotExist(Tables.BITHON_ALERT_OBJECT);

        new TableCreator(this.config, this.dslContext).partitionByExpressioin(StringUtils.format("toYYYYMMDD(%s)",
                                                                                                 Tables.BITHON_ALERT_CHANGELOG.SERVER_UPDATE_TIME.getName()))
                                                      .createIfNotExist(Tables.BITHON_ALERT_CHANGELOG);

        new TableCreator(this.config, this.dslContext).useReplacingMergeTree(true)
                                                      .replacingMergeTreeVersion(Tables.BITHON_ALERT_STATE.LAST_ALERT_AT.getName())
                                                      .partitionByExpressioin(null)
    ...
    public boolean updateAlert(AlertStorageObject oldObject, AlertStorageObject newObject, String operator) {
        return dslContext.insertInto(Tables.BITHON_ALERT_OBJECT)
                         ...
                         .set(Tables.BITHON_ALERT_OBJECT.CREATED_AT, oldObject.getCreatedAt())
                         .set(Tables.BITHON_ALERT_OBJECT.UPDATED_AT, new Timestamp(System.currentTimeMillis()))
                         .execute() > 0;
    }
    ...
    public boolean disableAlert(String alertId, String operator) {
        ...
        if (object != null) {
            return dslContext.insertInto(Tables.BITHON_ALERT_OBJECT)
                             ...
                             .set(Tables.BITHON_ALERT_OBJECT.LAST_OPERATOR, operator)
                             .set(Tables.BITHON_ALERT_OBJECT.CREATED_AT, object.getCreatedAt())
                             .set(Tables.BITHON_ALERT_OBJECT.UPDATED_AT, new Timestamp(System.currentTimeMillis()))
                             .execute() > 0;
        }
        return false;
    ...
    public <T> T executeTransaction(Callable<T> callable) {
        // Transaction is not supported by ClickHouse
        try {
