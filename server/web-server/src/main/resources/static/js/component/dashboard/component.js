class Dashboard {
    constructor(containerId, appName, schemaApi) {
        ...
        // Model
        this._chartLoadStatus = {};     // chart load status
        this._chartComponents = {};     // chart UI component
        this._chartDescriptors = {};    // chart configuration
        this._dimensions = {};
                ...
                schema => {
                    ...
                    if (schema.name === dataSourceFilter) {
                        ...
                        for (var index = 1; index < schema.dimensionsSpec.length; index++) {
                            dimension = schema.dimensionsSpec[index];
...
//                }
    ...
    refreshChart(chartId) {
        ...
        const chartLoadStatus = this._chartLoadStatus[chartId];
        if (chartLoadStatus != null) {
            interval.start = chartLoadStatus.lastLoadEnd;
        }
        const chartDescriptor = this._chartDescriptors[chartId];
        const chartComponent = this._chartComponents[chartId];
        chartComponent.load({
            incremental: chartLoadStatus != null,
            ...
            processResult: (data) => {
                this._chartLoadStatus[chartId] = {
                    lastLoadStart: interval.start,
                    lastLoadEnd: interval.end
                };
    ...
    /**
     * metric transformer at client side for UI
     *
     * eg. unit transformation
     */
