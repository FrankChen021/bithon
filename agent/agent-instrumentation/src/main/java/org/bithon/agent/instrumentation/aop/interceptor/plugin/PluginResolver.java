    ...
    private IPlugin loadPlugin(String pluginJarEntryName, JarClassLoader pluginClassLoader) {
        ...
        try {
            ...
            if (classFileMajorVersion == -1) {
                LOG.warn("Found plugin [{}], but skipped due to unrecognizable plugin class file version: [{}]", pluginName, t.getMessage());
            } else {
            ...
            LOG.warn(String.format(Locale.ENGLISH, "Failed to load plugin [%s]", pluginName), e);
            return null;
        }
    }
