 ...
 */
public class BrpcMetricMessageExporter implements IMessageExporter {
    private static final ILogAdaptor LOG = LoggerFactory.getLogger(BrpcMetricMessageExporter.class);
    ...
    public void export(Object message) {
        ...
        if (null == method) {
            LOG.warn("No service method found for entity: " + messageClass);
            return;
        }
        try {
            boolean isDebugOn = this.exporterConfig.getMessageDebug().getOrDefault(messageClass, false);
            if (isDebugOn) {
