...
/**
 * interceptor of {@link io.netty.util.ResourceLeakDetector#reportTracedLeak(String, String)}
 *
 * @author frank.chen021@outlook.com
 * @date 2021/5/13 5:32 下午
 */
public class ResourceLeakDetector$ReportTracedLeak extends BeforeInterceptor {

    @Override
    public void before(AopContext aopContext) {
        if (aopContext.getArgs().length >= 2) {
            ExceptionBuilder builder = ExceptionBuilder.builder()
                                                       .message(StringUtils.format("LEAK: %s.release() was not called before it's garbage-collected.",
                                                                                   aopContext.getArgs()[0]))
                                                       .stack(aopContext.getArgs()[1].toString())
